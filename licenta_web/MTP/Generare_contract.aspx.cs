...
namespace MTP
{
    ...
    {
        private const string ConnStr = "Server=tcp:cazare.database.windows.net,1433;Initial Catalog = cazare_camin; Persist Security Info=False;User ID = sqladmin; Password=Steluta1020.; MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=False;Connection Timeout = 30;";

        public SqlDataReader RetrieveStudentData(string inputNume)
        {
            SqlConnection connection = new SqlConnection(ConnStr);
            SqlCommand command = new SqlCommand("SELECT Nume, Prenume, Adresa, CNP, SerieCI, NrCI, NumeMama, NumeTata, Facultatea, AnStudiu, Specializarea, TeleofonParinte, TelefonStudent, Gen, NrMatricol, Judet,NrCamera, NrInregistrare FROM date_studenti WHERE Nume = @Nume", connection);
    ...
    public partial class Generare_contract : System.Web.UI.Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (reader.Read())
                {
                    ...
                    string NrCamera = reader["NrCamera"].ToString();
                    string NrInregistrare = reader["NrInregistrare"].ToString();
                    ...
                    CreateWordDocument(Nume, Prenume,  Adresa,  CNP,  SerieCI,  NrCI,  NumeMama,  NumeTata,  Facultatea,  AnStudiu,  Specializarea,  TelefonParinte,  TelefonStudent, Gen, NrMatricol, Judet, currentDate, NrCamera, NrInregistrare, @"C:\Users\daria\Desktop\contract_template.docx", @"C:\Users\daria\Desktop\contract_Nou.docx");
        ...
        private void CreateWordDocument(string Nume, string Prenume, string Adresa, string CNP, string SerieCI, string NrCI, string NumeMama, string NumeTata, string Facultatea, string AnStudiu, string Specializarea, string TeleofonParinte, string TelefonStudent, string Gen, string NrMatricol, string Judet, DateTime currentDate,string NrCamera, string NrInregistrare, object filename, object SaveAs)
        {
            ...
            if (File.Exists((string)filename))
            {
                ...
                this.FindAndReplace(wordApp, "<nr_camera>", NrCamera);
                this.FindAndReplace(wordApp, "<nr_inregistrare>", NrInregistrare);



                this.FindAndReplace(wordApp, "<data>", currentDate);
